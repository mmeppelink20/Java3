        ...
        <tr>
            <td>
                <% if (artist.getImages().length > 0) { %>
                    <img width="150" height="150" src="<%= artist.getImages()[0].getUrl()%>" alt=" <%= artist.getName() %>">
                <% } else { %>
            ...
            <td class="text-end"> <%= String.format("%,.0f", (double) artist.getFollowers().getTotal()) %> </td>
            <td class="text-end"> <%= artist.getPopularity() %> of 100</td>
            <td>
                <a class="btn btn-success" href="<%= artist.getExternalUrls().getExternalUrls().get("spotify") %>" role="button" target="_blank">See more info</a>
            </td>
        </tr>
        <% } %>
        </tbody>
